...
public class AsteroidController : MonoBehaviour {
	...
	public float numAsteroidsMin;
    public float numAsteroidsMax;
	...
	IEnumerator SpawnAsteroids(float time){
		...
		if(playerVector.magnitude > 30){
			int asteroidsToSpawn = (int)Mathf.Floor(Random.Range(numAsteroidsMin, numAsteroidsMax));
			for(int i = 0; i < asteroidsToSpawn; i++){
				Vector3 position = normalVector * Random.Range(-5, 5);
